...
import runGolden from './fixtures/wire/endpoints/run-flux2pro-callback.json';
import cancelGolden from './fixtures/wire/endpoints/task-cancel.json';
import detailIdGolden from './fixtures/wire/endpoints/task-detail-id.json';
import detailTokenGolden from './fixtures/wire/endpoints/task-detail-token.json';
import killGolden from './fixtures/wire/endpoints/task-kill.json';
import dynamicGolden from './fixtures/wire/requests/dynamic.json';
...
describe('run model wire contract', () => {
  it('matches the run golden path and body', async () => {
    ...
    expect(JSON.parse(transport.requests[0]?.body ?? '')).toEqual(runGolden);
...
describe('task management wire contract', () => {
  it('matches token and ID detail goldens', async () => {
    const transport = new FakeHttpTransport();
    transport.enqueueJson(
      200,
  ...
  it('matches cancel and kill goldens', async () => {
    const transport = new FakeHttpTransport();
    transport.enqueueJson(200, '{"result":true}');
    transport.enqueueJson(200, '{"result":true}');
    transport.enqueueJson(200, '{"result":true}');
    ...
    expect(JSON.parse(transport.requests[0]?.body ?? '')).toEqual(cancelGolden);
    expect(JSON.parse(transport.requests[1]?.body ?? '')).toEqual(killGolden);
